...
class WebSiteCrawler:
    ...
    def analyze_http_headers(self, url, headers):
        ...
        # Affichage console
        if missing_headers:
            print(f"  🔴 Headers manquants: {' / '.join(missing_headers[:5])}")
            if len(missing_headers) > 5:
                print(f"      ... et {len(missing_headers) - 5} autres")
    
        if info_disclosure:
            print(f"  🟠 Info disclosure: {', '.join([h['header'] for h in info_disclosure])}")
    ...
    def normalize_url(self, url):
        """Normalise une URL pour éviter les doublons - VERSION MINIMALE"""
        parsed = urlparse(url)
        
        # ✅ Suppression UNIQUEMENT des fragments (#...)
    ...
    def crawl(self, start_url, depth=0):
        ...
        # ✅ Tracker des URLs déjà ajoutées à la queue
        queued_urls = {self.normalize_url(start_url)}
            ...
            if 'text/html' in content_type or 'application/xhtml' in content_type:
                try:
                    ...
                    for resource in resources:
                        ...
                        # ✅ Normaliser AVANT vérification
                        normalized_resource = self.normalize_url(resource)
                        
                        # ✅ Vérifier visited_urls ET queued_urls
                        if normalized_resource in self.visited_urls:
                            continue
                        if normalized_resource in queued_urls:
                            continue
                        ...
                        if self.is_same_domain(normalized_resource):
                            ...
                            if self.is_interesting_file(normalized_resource):
                                ...
                                # ✅ Ajouter à la queue ET au tracker
                                queue.append((normalized_resource, current_depth + 1))
                                queued_urls.add(normalized_resource)
                            else:
    ...
    def explore_directories(self):
        ...
        # ✅ SUPPRESSION de max_iterations (pas de limite artificielle)
        ...
        if self.headers_info:
            ...
            for url, info in list(self.headers_info.items()):
                ...
                if info.get('missing_security_headers'):
                    ...
                    missing_str = ' / '.join(info['missing_security_headers'])
                ...
                if info.get('info_disclosure'):
                    ...
                    disclosure_str = ', '.join([h['header'] for h in info['info_disclosure']])
                ...
                if info.get('suspicious_headers'):
                    ...
                    print(f"   🟡 Suspects: {len(info['suspicious_headers'])} header(s)")
            ...
            if self.headers_info:
                ...
                for url, info in list(self.headers_info.items()):
                    ...
                    if info.get('missing_security_headers'):
                        ...
                        missing_str = ' / '.join(info['missing_security_headers'])
                        f.write(f"   🔴 Manquants: {missing_str}\n")
                        total_missing += len(info['missing_security_headers'])
                    ...
                    if info.get('info_disclosure'):
                        ...
                        disclosure_str = ', '.join([h['header'] for h in info['info_disclosure']])
                        f.write(f"   🟠 Disclosure: {disclosure_str}\n")
                        total_disclosure += len(info['info_disclosure'])
                    ...
                    if info.get('suspicious_headers'):
                        ...
                        f.write(f"   🟡 Suspects: {len(info['suspicious_headers'])} header(s)\n")
                        total_suspicious += len(info['suspicious_headers'])
